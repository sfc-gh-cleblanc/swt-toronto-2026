...
render_technologies_used([
    ...
    },
    {
        ...
        "description": "Snowflake's collaborative AI workspace. Access your agent alongside other tools for team-based data exploration.",
        "icon": "group",
    },
    {
...
render_coco_prompt(
    "5.2",
    "Grant CoWork Access",
    "Grant the SNOWFLAKE service role USAGE on iceberg_lab_db and its analytics schema, "
    "plus SELECT on both quotes_vw and the quotes_sv semantic view. "
    "This enables CoWork to access the agent. "
    "Note: if you get a 'Role does not exist' error on a trial account, skip this step.",
    sql="""\
USE ROLE ACCOUNTADMIN;
...
GRANT SELECT ON SEMANTIC VIEW iceberg_lab_db.analytics.quotes_sv TO ROLE SNOWFLAKE;""",
    note="The SNOWFLAKE service role enables CoWork access. If you get a 'Role does not exist' error on a trial account, skip this — the agent is still accessible via AI & ML > Agents.",
)

render_explanation("CoWork and the SNOWFLAKE role", """
**Snowflake CoWork** is the collaborative AI workspace available in Enterprise and Business Critical accounts.
To make your agent accessible in CoWork, grant the `SNOWFLAKE` service role access to the underlying objects.

The `SNOWFLAKE` role is a system role — you don't create it. It's used internally by CoWork to
execute queries on behalf of users. Masking policies still apply based on the user's active role.

**Access methods for Cortex Agents:**

| Method | Use case |
|--------|----------|
| **CoWork** | Collaborative data exploration with your team |
| **AI & ML > Agents** | Direct access in Snowsight (works on all account types) |
| **REST API** | Embed the agent in external applications |
| **SQL (CORTEX.AGENT)** | Call programmatically from SQL or stored procedures |
""")
...
1. In the left nav, click **AI & ML** then **Agents**
2. Find **Canadian Insurance Quotes Analyst** in the list and click **Open**
...
st.markdown("Try these questions to exercise the agent's capabilities:")

questions = [
    ("Product breakdown", "What are the top 5 insurance products by number of quotes?"),
    ...
    ("Credit score insight", "Compare average premiums across credit score bands — are higher scores getting better rates?"),
...
2. Ask: *"Show me the top 5 customers by total premium"*
...
The same Iceberg data, the same agent, the same question — but governance controls
who sees what, all the way from S3 through the AI response.
""")

st.markdown("---")

st.markdown("#### Use with Snowflake CoWork")

st.markdown("""
If your account supports CoWork (Enterprise or Business Critical), navigate to **CoWork**
in the left sidebar and try these questions:
""")

cowork_questions = [
    "Give me an overview of our insurance quote portfolio — total quotes, average premium, and top product types.",
    "Which regions show the highest quote activity and what are their average premiums?",
    "Are homeowners getting better insurance rates than renters? Break it down by product type.",
    "What trends do you see in quote frequency by marital status?",
    "Summarize the relationship between credit scores and insurance premiums across our portfolio.",
]

for q in cowork_questions:
    with st.container(border=True):
...
render_key_concepts([
    ...
    },
    {
        ...
        "definition": "Snowflake's collaborative AI workspace. Provides a conversational interface to agents, with support for team collaboration, conversation history, and multi-agent workflows.",
    },
    {
...
render_what_you_built([
    ...
    "CoWork access for collaborative data exploration",
    "End-to-end masking enforcement from Iceberg through the agent",
    "Tested role-based access — analysts see masked PII, engineers see full values",
])
